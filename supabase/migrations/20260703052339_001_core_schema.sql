/*
# Core Schema — Garment Manufacturing ERP

## Summary
Creates the foundational schema for a single-tenant garment manufacturing ERP system.
Replaces localStorage with PostgreSQL for persistent, relational data storage.

## New Tables
1. `app_settings` — company name and default unit
2. `fabrics` — fabric master with name and unit
3. `fabric_colors` — per-fabric colour entries with roll/stock/used tracking
4. `raw_materials` — raw material master (thread, zip, button, label, etc.)
5. `articles` — article (style) master with fabric assignment
6. `article_size_consumption` — per-size fabric consumption for each article
7. `article_material_consumption` — per-material consumption sheet for costing
8. `history_events` — audit log of user actions
...
## Notes
- This is a single-tenant app with no sign-in screen; the anon key is used for all operations.
- `USING (true)` is intentional for a shared/public single-tenant dataset.
...
-- ARTICLE SIZE CONSUMPTION (fabric per size)
...
-- ARTICLE MATERIAL CONSUMPTION (BOM / costing sheet)
...
-- Seed default app_settings (only if empty)
