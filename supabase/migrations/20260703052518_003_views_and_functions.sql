/*
# Views and Helper Functions — Garment Manufacturing ERP

## Summary
Creates SQL views for dashboards and reporting, plus helper functions for cost/production calculations.

## New Views
1. `v_lot_summary` — lot with article/fabric names, planned production, color count
2. `v_lot_cost_summary` — full costing: total material cost, CPP, revenue, profit
3. `v_cutting_summary` — per-lot total cut PCS and fabric used
4. `v_finishing_summary` — per-lot total finished PCS
5. `v_pressing_summary` — per-lot total pressed PCS
6. `v_packing_summary` — per-lot total packed PCS and boxes
7. `v_dispatch_summary` — per-lot total dispatched boxes

## New Functions
1. `get_lot_cut_pcs(lot_id)` — total PCS cut for a lot
2. `get_lot_finished_pcs(lot_id)` — total PCS finished for a lot
3. `get_lot_pressed_pcs(lot_id)` — total PCS pressed for a lot
4. `get_lot_packed_pcs(lot_id)` — total PCS packed for a lot
5. `get_lot_dispatched_boxes(lot_id)` — total boxes dispatched for a lot

## Security
- Views inherit table RLS.
- Functions are SECURITY INVOKER (default).
*/
...
SELECT
  ...
  l.lot_no,
  l.status,
  l.planned_production,
  l.selling_price_per_pcs,
  l.created_at,
  a.id AS article_id,
...
SELECT
  ...
  COALESCE(
    (SELECT SUM(amc.consumption * rm.purchase_price * l.planned_production)
     FROM article_material_consumption amc
     JOIN raw_materials rm ON rm.id = amc.material_id
     WHERE amc.article_id = l.article_id), 0
  ) AS total_lot_cost,
  CASE WHEN l.planned_production > 0 THEN
    COALESCE(
      (SELECT SUM(amc.consumption * rm.purchase_price)
       FROM article_material_consumption amc
       JOIN raw_materials rm ON rm.id = amc.material_id
       WHERE amc.article_id = l.article_id), 0
    )
...
SELECT
  ...
  COALESCE(SUM(ce.fabric_used), 0) AS total_fabric_used,
  COALESCE(SUM(ccs.qty), 0) AS total_cut_pcs
FROM cutting_entries ce
LEFT JOIN cutting_color_sizes ccs ON ccs.cutting_id = ce.id
GROUP BY ce.lot_id;
...
SELECT
  ...
  COALESCE(SUM(fcs.qty), 0) AS total_finished_pcs
FROM finishing_entries fe
LEFT JOIN finishing_color_sizes fcs ON fcs.finishing_id = fe.id
GROUP BY fe.lot_id;
...
SELECT
  ...
  COALESCE(SUM(pcs2.qty), 0) AS total_pressed_pcs
FROM pressing_entries pe
LEFT JOIN pressing_color_sizes pcs2 ON pcs2.pressing_id = pe.id
GROUP BY pe.lot_id;

CREATE OR REPLACE VIEW v_packing_summary AS
SELECT
  pa.lot_id,
  COUNT(DISTINCT pa.id) AS entry_count,
  COALESCE(SUM(psb.boxes), 0) AS total_boxes,
  COALESCE(SUM(pbc.pcs * psb.boxes), 0) AS total_packed_pcs
FROM packing_entries pa
LEFT JOIN packing_size_boxes psb ON psb.packing_id = pa.id
LEFT JOIN packing_box_contents pbc ON pbc.packing_size_box_id = psb.id
GROUP BY pa.lot_id;
...
SELECT
  ...
  COALESCE(SUM(db.boxes), 0) AS total_dispatched_boxes
FROM dispatch_entries de
LEFT JOIN dispatch_boxes db ON db.dispatch_id = de.id
GROUP BY de.lot_id;
