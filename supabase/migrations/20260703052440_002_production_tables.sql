/*
# Production Tables — Lots, Cutting, Finishing, Pressing, Packing, Dispatch

## Summary
Creates all production workflow tables for the garment manufacturing ERP.

## New Tables
1. `lots` — production lot master with article, fabric, status, selling price
2. `lot_colors` — which fabric colors are selected for a lot
3. `lot_color_plans` — per-color fabric allocation (planned KG, rolls selected)
4. `lot_size_plans` — per-size planned PCS distribution
5. `cutting_entries` — cutting stage entries (date, fabric used, notes)
6. `cutting_color_sizes` — per-color per-size qty cut
7. `finishing_entries` — finishing stage entries
8. `finishing_color_sizes` — per-color per-size qty finished
9. `pressing_entries` — pressing stage entries
10. `pressing_color_sizes` — per-color per-size qty pressed
11. `packing_entries` — packing stage entries
12. `packing_size_boxes` — per-size box configuration
13. `packing_box_contents` — per-color pcs per box
14. `dispatch_entries` — dispatch entries with party and invoice
15. `dispatch_boxes` — which packing boxes were dispatched and how many
...
## Notes
- All tables link back to lot via lot_id (indexed).
- ON DELETE CASCADE from lot to child tables.
*/
...
-- LOT COLOR PLANS (per color: how many rolls selected, how much fabric planned)
...
-- LOT SIZE PLANS (per size: planned PCS)
...
-- CUTTING COLOR SIZES (per cutting entry, per color, per size)
...
-- PACKING SIZE BOXES (per packing entry, per size)
...
-- PACKING BOX CONTENTS (per size-box, per color)
...
-- DISPATCH BOXES (which packing boxes were dispatched)
...
-- updated_at triggers for production tables
DO $$ 
DECLARE tbl text;
BEGIN
